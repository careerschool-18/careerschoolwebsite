...
const mockEmployees = [
  { id: 1, name: "Krishna Kumar", role: "Lead Mentor", email: "krishna@careerschool.com", status: "Active", joinDate: "10 Jan 2024", password: "krishna@CS2024" },
  { id: 2, name: "Neha Sharma", role: "HR Manager", email: "neha@careerschool.com", status: "Active", joinDate: "15 Jan 2024", password: "neha@CS2024" },
  { id: 3, name: "Rohan Verma", role: "React Instructor", email: "rohan.v@careerschool.com", status: "Active", joinDate: "01 Feb 2024", password: "rohan@CS2024" },
  { id: 4, name: "Priya Patel", role: "Python Instructor", email: "priya.p@careerschool.com", status: "Active", joinDate: "12 Feb 2024", password: "priya@CS2024" },
  { id: 5, name: "Amit Singh", role: "Admissions Head", email: "amit.s@careerschool.com", status: "Active", joinDate: "20 Feb 2024", password: "amit@CS2024" },
  { id: 6, name: "Anjali Gupta", role: "Career Coach", email: "anjali.g@careerschool.com", status: "Active", joinDate: "05 Mar 2024", password: "anjali@CS2024" },
  { id: 7, name: "Suresh Raina", role: "Data Science Mentor", email: "suresh.r@careerschool.com", status: "Active", joinDate: "15 Mar 2024", password: "suresh@CS2024" },
  { id: 8, name: "Vikram Malhotra", role: "DevOps Engineer", email: "vikram.m@careerschool.com", status: "Active", joinDate: "01 Apr 2024", password: "vikram@CS2024" },
  { id: 9, name: "Sneha Reddy", role: "Student Coordinator", email: "sneha.r@careerschool.com", status: "Active", joinDate: "10 Apr 2024", password: "sneha@CS2024" },
  { id: 10, name: "Rajesh Koothrapali", role: "Astrophysics Instructor", email: "rajesh.k@careerschool.com", status: "Inactive", joinDate: "15 Apr 2024", password: "rajesh@CS2024" },
  { id: 11, name: "Deepika Padukone", role: "Public Relations", email: "deepika@careerschool.com", status: "Active", joinDate: "01 May 2024", password: "deepika@CS2024" },
  { id: 12, name: "Ranbir Kapoor", role: "Soft Skills Trainer", email: "ranbir.k@careerschool.com", status: "Active", joinDate: "10 May 2024", password: "ranbir@CS2024" },
  { id: 13, name: "Alia Bhatt", role: "UI/UX Mentor", email: "alia.b@careerschool.com", status: "Active", joinDate: "20 May 2024", password: "alia@CS2024" },
  { id: 14, name: "Varun Dhawan", role: "Marketing Specialist", email: "varun.d@careerschool.com", status: "Inactive", joinDate: "01 Jun 2024", password: "varun@CS2024" },
  { id: 15, name: "Kriti Sanon", role: "Frontend Instructor", email: "kriti.s@careerschool.com", status: "Active", joinDate: "15 Jun 2024", password: "kriti@CS2024" },
  { id: 16, name: "Ayushmann Khurrana", role: "Content Writer", email: "ayushmann.k@careerschool.com", status: "Active", joinDate: "01 Jul 2024", password: "ayushmann@CS2024" },
  { id: 17, name: "Rajkummar Rao", role: "Backend Mentor", email: "rajkummar.r@careerschool.com", status: "Active", joinDate: "10 Jul 2024", password: "rajkummar@CS2024" },
  { id: 18, name: "Bhumi Pednekar", role: "Operations Lead", email: "bhumi.p@careerschool.com", status: "Active", joinDate: "20 Jul 2024", password: "bhumi@CS2024" },
];
...
export default function SuperAdminDashboard() {
  ...
  const [overviewSubView, setOverviewSubView] = useState("main"); // "main" | "employees" | "batches" | "students"
  ...
  useEffect(() => {
    ...
    const localEmployees = localStorage.getItem("mock_employees");
    if (localEmployees) {
    ...
    const syncFromLocalStorage = async () => {
      ...
      const updatedEmployees = localStorage.getItem("mock_employees");
      if (updatedEmployees) {
        setEmployees(JSON.parse(updatedEmployees));
      }
    };
  ...
  // State variables for changing employee or student passwords
  const [changePasswordTarget, setChangePasswordTarget] = useState(null); // { type: 'employee' | 'student', id: string|number, name: string, password: string }
  ...
  const handleTargetPasswordSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      // Update local state
      if (changePasswordTarget.type === "employee") {
        setEmployees(prev => {
          const updated = prev.map(emp =>
            emp.id === changePasswordTarget.id
              ? { ...emp, password: targetPassword.newPassword }
              : emp
          );
  ...
  const handleUsernameSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:8080/api/superadmin/update-username", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        setToast(`Username updated to "${targetUsername}" successfully!`);
        setUsernameInput("");
      } else {
        const errorText = await response.text();
        setToast(`Error: ${errorText}`);
      }
    } catch (err) {
      // Fallback state update for standalone frontend usage
      setAdmin(prev => ({
        ...prev,
        name: targetUsername,
      }));
      setToast(`Username updated locally: "${targetUsername}" (Server offline)`);
      setUsernameInput("");
    }
  };
  ...
  const handleAdminPasswordSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:8080/api/superadmin/update-password", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      setToast("Password updated locally (Server offline)");
      setPassword({ newPassword: "", confirmPassword: "" });
    }
  };
  ...
  // Calculate total pages for employees
  const totalEmployeePages = Math.ceil(employees.length / itemsPerPage);
  const employeeStartIndex = (employeePage - 1) * itemsPerPage;
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50">
            ...
            <>
              {overviewSubView === "main" && (
                 <div className="grid md:grid-cols-3 gap-5 animate-fade-in">
                  ...
                  <div
                    onClick={() => {
                      setOverviewSubView("employees");
                      setEmployeePage(1);
                    }}
                    className="bg-white p-6 rounded-2xl shadow-sm border cursor-pointer hover:shadow-md hover:border-blue-400 hover:scale-[1.01] transition-all duration-200 group"
                  >
                    <FiUser className="text-blue-600 mb-4 group-hover:scale-110 transition-transform" size={28} />
                ...
                <div className="bg-white rounded-2xl shadow-sm border p-6 md:p-8 animate-fade-in">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-gray-900">Employees Directory</h3>
                        <p className="text-sm text-gray-500 mt-1">Manage and view system users</p>
                      </div>
                    </div>
                      ...
                      <tbody className="divide-y text-sm text-gray-700">
                        {paginatedEmployees.map((emp) => (
                          <tr key={emp.id} className="hover:bg-gray-50/70 transition-colors">
                            <td className="py-4 px-4 font-semibold text-gray-900">{emp.name}</td>
                            <td className="py-4 px-4 text-gray-500">{emp.role}</td>
                            ...
                            <td className="py-4 px-4 text-gray-500">{emp.joinDate}</td>
                  ...
                  {totalEmployeePages > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 pt-6 border-t text-sm text-gray-500">
                      <div>
                        Showing <span className="font-medium text-gray-900">{employeeStartIndex + 1}</span> to{" "}
                        <span className="font-medium text-gray-900">
                          {Math.min(employeeStartIndex + itemsPerPage, employees.length)}
                        </span>{" "}
                        of <span className="font-medium text-gray-900">{employees.length}</span> employees
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        {Array.from({ length: totalEmployeePages }, (_, idx) => (
                          <button
                            key={idx + 1}
                            onClick={() => setEmployeePage(idx + 1)}
                        ...
                        <button
                          onClick={() => setEmployeePage(prev => Math.min(prev + 1, totalEmployeePages))}
                ...
                <form onSubmit={handleUsernameSubmit} className="space-y-5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full mt-2 border rounded-xl px-4 py-3 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors outline-none"
                      value={usernameInput}
        ...
        {changePasswordTarget && (() => {
          const targetUser = changePasswordTarget.type === "employee"
            ? employees.find(e => e.id === changePasswordTarget.id)
            : (students[selectedBatch?.id] || []).find(s => s.id === changePasswordTarget.id);
